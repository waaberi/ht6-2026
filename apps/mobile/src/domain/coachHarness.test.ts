...
import { planCoachAction } from './coachHarness';
...
test('every Coach tool routes into a concrete editor or camera plan', () => {
  ...
  assert.deepEqual(planCoachAction(action({ tool: 'expand', canvasTransform: { expansion: { top: 0, right: 10, bottom: 0, left: 0 } } }), transform), {
    kind: 'expand', direction: 'right', prompt: 'Extend the scene naturally into the new canvas.',
  });
