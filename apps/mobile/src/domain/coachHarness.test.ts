...
test('every Coach tool routes into a concrete editor or camera plan', () => {
  ...
  assert.deepEqual(planCoachAction(action({ tool: 'amplify', target: { x: 0, y: 0, width: 0.2, height: 0.2 }, prompt: 'Add a kite' }), transform), {
    kind: 'generative', target: { x: 0, y: 0, width: 0.2, height: 0.2 }, prompt: 'Add a kite',
  });
...
test('incomplete AI actions fail instead of silently doing the wrong edit', () => {
  ...
  assert.throws(() => planCoachAction(action({ tool: 'amplify', target: { x: 0, y: 0, width: 0.2, height: 0.2 } }), identityCanvasTransform()), /incomplete/);
