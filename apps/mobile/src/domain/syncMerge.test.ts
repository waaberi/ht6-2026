...
test('sync progress preserves photos created after a sync started', () => {
  const result = mergeSyncProgress([photo('new', 'new-v1', 'queued'), photo('old')], [photo('old', 'old-v1', 'syncing')]);
...
test('sync progress never overwrites a newer local version', () => {
  const result = mergeSyncProgress([photo('old', 'old-v2', 'queued')], [photo('old', 'old-v1', 'synced')]);
